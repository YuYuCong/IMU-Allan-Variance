...
void imu::AllanAcc::CalculateAllanVariance() {
  std::cout << m_name << " "
...
std::vector<double> imu::AllanAcc::GetTimestamp() {
  double period = getAvgPeriod();
...
std::vector<double> imu::AllanAcc::calcThetas(const double freq) {
...
std::vector<double> imu::AllanAcc::getLogSpace(float a, float b) {
